 ...
 titlePanel('Ruyesh Irrigation Calculator'),
  ...
  sidebarLayout(
    ...
    sidebarPanel(
     ...
     uiOutput('Crops'),
#     selectInput('crop', 'Crop', CropData$CROP),
#     selectInput('GHcrop', 'GreenhouseCrop', intersect(Stock_A$CROP, CropData$CROP)),
...
#     actionButton('show', 'Show Fertilizer Table'),
...
# SrcStrng <- reactive({paste('http://api.weatherstack.com/current?access_key=7b2b37e649d0f49311fc48763b1fb0b4&query=', cityLoc()[1],',', cityLoc()[2], sep='')
# })

#  SrcStrng <- reactive({paste('http://api.openweathermap.org/data/2.5/weather?lat=',
#                              cityLoc()[1], '&lon=', cityLoc()[2], '&appid=9d34eadb7918714c45307ac75569ecb0',sep='')
#  })
...
# plotdata <- eventReactive(input$show, {
#    ClimData()
#  }, ignoreNULL = TRUE)

# FertWei <- eventReactive(input$show, {
#    as.data.frame(Stock_A[Stock_A$CROP == input$crop,])
#  }, ignoreNULL = TRUE)

#  output$ruyesh <- renderText({
#    'Ruyesh for you'
#  })
